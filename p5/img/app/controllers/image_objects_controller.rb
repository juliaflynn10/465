class ImageObjectsController < ApplicationController
  ...
  def index
    ...
    @imgs = ImageObject.select{|i| i.public}
    @privimgs = ImageObject.select{|i| i.user_id == current_user.id}
    @sharimgs = ImageObject.select{|i| ImageUser.select{|u| current_user.id == u.user_id }
    @users = User.all  
  end
